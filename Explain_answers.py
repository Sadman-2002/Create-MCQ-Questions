...
- Provide explanations for why EACH WRONG answer is incorrect
        ...
        for sent in selected_sentences:
            ...
            for i, opt in enumerate(all_options):
                if i == correct_index:
                    ...
                    wrong_explanations.append(f"This word does not fit the context of the sentence from the source material.")
            
            questions.append({
                "question": f"Complete the sentence: {question_text}",
                "options": all_options,
                "correct_index": correct_index,
                "explanation": f"The correct answer is '{blank_word}' as it appears in the original text: {sent[:100]}...",
...
<body>
        ...
        <div id="uploadScreen">
            <div class="info-box">
                ...
                <ul style="margin-left: 20px; margin-top: 10px;">
                    ...
                    <li>Get detailed explanations for correct AND wrong answers ✍</li>
                </ul>
            </div>
    ...
    <script>
        ...
        function checkAnswer(selected) {
            ...
            // Build detailed explanation HTML
            let explanationHTML = '<div class="explanation-container">';
            explanationHTML += '<div class="explanation-header">📚 Detailed Explanation</div>';
            
            // Correct answer explanation (GREEN)
            explanationHTML += '<div class="correct-explanation">';
            explanationHTML += '<strong>✅ Correct Answer: ' + String.fromCharCode(65 + q.correct_index) + ') ' + q.options[q.correct_index] + '</strong><br>';
            explanationHTML += '<div style="margin-top: 10px;">' + q.explanation + '</div>';
            explanationHTML += '</div>';
            
            // Wrong answers explanations (RED)
            if (q.wrong_explanations && q.wrong_explanations.length > 0) {
                q.options.forEach((opt, i) => {
                    if (i !== q.correct_index && q.wrong_explanations[i] && q.wrong_explanations[i].trim() !== '') {
                        explanationHTML += '<div class="wrong-explanation">';
                        explanationHTML += '<strong>❌ Why "' + String.fromCharCode(65 + i) + ') ' + opt + '" is wrong:</strong><br>';
                        explanationHTML += '<div style="margin-top: 10px;">' + q.wrong_explanations[i] + '</div>';
                        explanationHTML += '</div>';
                    }
                });
            }
...
if __name__ == '__main__':
    ...
    print("   ✍  Detailed explanations for correct AND wrong answers")
    print("   🎯 Green explanations for correct, Red for wrong answers")
