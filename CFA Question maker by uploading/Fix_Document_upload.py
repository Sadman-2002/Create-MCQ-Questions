...
def extract_text_from_pdf(file_path):
    ...
    try:
        ...
        if len(text.strip()) > 100:
            ...
            print("  ⚠️ PyPDF2 extracted minimal text, trying alternative...")
    except ImportError:
    ...
    try:
        ...
        if len(text.strip()) > 100:
            ...
            print("  ⚠️ PyMuPDF extracted minimal text")
    except ImportError:
    ...
    if len(text.strip()) < 100:
        raise Exception("Could not extract sufficient text from PDF. The PDF might be image-based or encrypted. Try converting it to text first.")
    ...
    try:
        ...
        try:
            with open(file_path, 'r', encoding='latin-1') as file:
                text = file.read()
                print(f"  ✅ Extracted {len(text)} characters (latin-1)")
                return text
        except Exception as e:
            print(f"  ❌ TXT error: {e}")
            return ""
...
def extract_text_from_file(file_path, filename):
    ...
    print(f"\n📄 Processing {extension.upper()} file: {filename}")
...
1. **reference_keywords**: Array of 3-5 specific keywords/phrases that appear in the source material and relate to this question.
...
{{
  "questions": [
    {{
      ...
      "explanation": "Professional reasoning",
...
"""
        ...
        try:
            response = client.chat.completions.create(
                model="gpt-3.5-turbo",
                messages=[
                    {"role": "system", "content": "You are a CFA question writer. Include reference_keywords array. Return ONLY valid JSON."},
...
def generate_fallback_cfa_mcqs(text, num_questions=10, filename="document"):
    ...
    for i in range(min(num_questions, len(sentences))):
        ...
        question = {
            ...
            "wrong_explanations": ["", f"Violates Reading {reading_num} principles.", f"Judgment is critical per LOS.", f"Dismisses key factors."],
...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    #question { font-size: 1.28em; color: #0f2027; line-height: 1.85; margin: 18px 0; font-weight: 500; }
    ...
    .stat-label { color: #2c5364; font-size: 0.95em; margin-top: 6px; font-weight: 600; }
...
<body>
  <div id="mainContainer">
    ...
    <p class="subtitle">Upload PDF • Clickable References • View Source Material</p>

    <div id="uploadScreen">
      <div class="info-box">
        <strong>✅ PDF Upload Fixed - Multiple Methods:</strong>
        <ul>
          <li><strong>📄 PDF Support:</strong> PyPDF2, pdfplumber, PyMuPDF (3 fallback methods)</li>
          <li><strong>📁 All Formats:</strong> PDF, Word, PowerPoint, Excel, TXT</li>
          <li><strong>📏 Large Files:</strong> Up to 50MB supported</li>
          <li><strong>🔗 Clickable Links:</strong> View source material from your document</li>
          <li><strong>🎲 Randomized:</strong> Correct answers shuffled</li>
        </ul>
      </div>
      ...
      <div class="upload-area" id="uploadArea">
        ...
        <div style="color: #2c5364; font-size: 1em; margin-top: 12px; font-weight: 600;">
          PDF, Word, PowerPoint, Excel, TXT (max 50MB)
        </div>
    ...
    <div id="loadingScreen">
      ...
      <div style="text-align: center; color: #2c5364; font-size: 1.3em; margin: 25px; font-weight: 600;">
        🧠 Processing your PDF and creating questions...<br>
        <span style="font-size: 1em; color: #6c757d; margin-top: 12px; display: block;">
          Extracting text and generating interactive references
        </span>
      </div>
    </div>
  ...
  <script>
    ...
    function handleFileSelect(file) {
      ...
      if (!allowedExts.includes(ext)) {
        showError('Invalid file type. Please upload: PDF, Word, PowerPoint, Excel, or TXT');
        return;
      }
      ...
      document.getElementById('uploadText').innerHTML = `✅ ${file.name}<br><span style="font-size: 0.8em;">${(file.size / 1024 / 1024).toFixed(2)} MB</span>`;
      uploadBtn.disabled = false;
      showSuccess('File ready to upload! Click "Generate Questions"');
    }
    ...
    function loadQuestion() {
      ...
      let refHTML = '<div class="reading-reference-title">📚 Reference</div>';
      
      if (q.reading_reference) {
        refHTML += '<div class="reading-reference-item"><strong>📖</strong> ' + q.reading_reference + '</div>';
      }
      if (q.los_reference) {
        refHTML += '<div class="reading-reference-item"><strong>🎯</strong> ' + q.los_reference + '</div>';
      }
    ...
    function checkAnswer(selected) {
      ...
      q.options.forEach((opt, i) => {
        ...
        if (i === q.correct_index) {
          ...
          html += (q.explanation_with_reference || q.explanation || 'Correct answer');
          
          if (q.reference_id) {
            html += '<br><br><a class="reference-link" onclick="openReferenceModal(\'' + q.reference_id + '\', \'' + (q.source_filename || 'document') + '\')">🔗 View Source Material</a>';
          }
    ...
    function showResults() {
      ...
      let msg = percent >= 70 ? '🎓 PASS! Excellent work!' : 
                percent >= 50 ? '📚 Good effort! Review and try again.' : 
                '💪 Keep studying!';
    ...
    try:
        ...
        if not text or len(text.strip()) < 50:
            if file_path and os.path.exists(file_path):
                os.remove(file_path)
            return jsonify({
                'error': 'Could not extract enough text. If this is a PDF, it may be image-based. Try: 1) Converting to text-based PDF, 2) Using OCR, 3) Uploading as Word/TXT format'
            }), 400
...
if __name__ == '__main__':
    print("\n" + "="*70)
    print("🎓 CFA Question Generator - PDF FIXED VERSION")
    print("="*70)
    ...
    print("   pip install python-docx python-pptx openpyxl")
    print("   pip install openai flask werkzeug")
    print("\n🔧 PDF Extraction Methods:")
    print("   1. PyPDF2 (primary)")
    print("   2. pdfplumber (fallback)")
    print("   3. PyMuPDF (fallback)")
    print("\n🎯 Features:")
    print("   • Multi-method PDF extraction")
    print("   • Up to 50MB file size")
    print("   • Clickable source references")
    print("   • Shuffled answers")
    print("   • 1-1000 questions")
